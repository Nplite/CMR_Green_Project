    ...
    def process_frame(self, frame, model):
        ...
        try:
            ...
            if self.frame_count > 20:
                combined_frame2, person_detected, _ = person_detection_ROI3(frame=combined_frame2, roi_points= self.roi_2_pts_np, model= self.model)
    ...
    def process_motion(self, frame, combined_frame1, combined_frame2, combined_frame3, motion_in_roi1, 
        ...
        if (motion_in_roi1 and person_detected) or (person_in_roi3 and person_detected):
            ...
            if time.time() <= self.recording_end_time:
                ...
                # # Optimized threading code using ThreadPoolExecutor
                # with ThreadPoolExecutor(max_workers=3) as executor:
                #     futures = [
                #         executor.submit(self.mongo_handler.save_snapshot_to_mongodb, self.snapshot_url, start_time, self.camera_id),
                #         executor.submit(self.mongo_handler.save_video_to_mongodb, self.video_url, start_time, self.camera_id),
                #         executor.submit(self.email.send_alert_email, self.snapshot_path, self.video_url, self.camera_id)
                #     ]
